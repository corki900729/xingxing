...
<div class="pd-20">
    <!--<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="member_add('添加系部','add','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加系部</a></span>  </div>-->
        ...
        <form action="<?php echo U('daan');?>" method="get">
        <div class="text-c"> 状态：
            <select name="flg" class="input-text" style="width:250px">
                <option value="0">请选择状态</option>
                <option value="1" <?php if($flg == 1): ?>selected<?php endif; ?>>正确</option>
                <option value="-2" <?php if($flg == -2): ?>selected<?php endif; ?>>错误</option>
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                    ...
                    <td><img style="height: 80px;width: 80px" src="<?php echo ($vo["headurl"]); ?>"/></td>
                    <td><?php echo ($vo["daan"]); ?></td>
                    <td>
                        <?php if($vo['flg'] == 1): ?><span style="color: #00b7ee">正确</span>
                            <?php elseif($vo['flg'] == -2): ?>
                            <span style="color: red">错误</span>&nbsp;&nbsp;&nbsp;
                            <?php elseif($vo['flg'] == -1): ?>
                            <span onclick="shenhe(<?php echo ($vo["id"]); ?>,1)" style="color: #00b7ee">回答正确</span>
                            <span onclick="shenhe(<?php echo ($vo["id"]); ?>,-2)" style="color: red">回答错误</span><?php endif; ?>
                    </td>
                    <td class="td-manage">
                        <a title="编辑" href="<?php echo U('edit_dati',array('id'=>$vo['id']));?>" class="ml-5"
                           style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
                        <a title="删除" href="<?php echo U('del_daan',array('id'=>$vo['id']));?>" class="ml-5"
                           style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
                        <!--<a title="答题情况" href="<?php echo U('info_dati',array('id'=>$vo['id']));?>" class="ml-5">答题情况</a>-->
                    </td>
...
<script type="text/javascript">
    ...
    function shenhe(id,flg) {
        $.ajax({
            type: 'post',
            data: {id: id,flg:flg},
            url: "<?php echo U('Join/daan_shenhe');?>",
